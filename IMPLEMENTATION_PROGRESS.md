...
> 最后更新：2026-08-09；当前分支：`toon-custom`
...
## 2026-08-09 第二十四批：新项目扫描与视频生成准备度门禁
...
- 同一检查在服务端入队执行，位置早于预算预留和供应商提交；活动任务仍由原持久队列去重复用。
- huobao 新增的失败终态、缩略图/海报帧和工作台位置记忆经对照已覆盖或不匹配当前架构，本轮不复制无许可证源码。
- 新来源中的动作拍点、配方复用、代表镜头先审、视觉漂移和节奏反馈闭环进入候选清单，等待独立数据契约和明确验证路径。

验证：根 `yarn lint`、后端生产构建、前端 Vite + `vue-tsc`、知识地图/索引和 12 来源台账校验通过。没有启动 Toonflow 服务，没有调用生成 API 或产生费用。

## 2026-08-09 第二十五批：动作拍点闭环与视频海报帧

参考来源：Jellyfish `a9678194ddf2d9be3ccbe78d4287d87d5089e123` 的动作拍点产品设计；huobao-drama `99e66809` / `a1b8220f21d554e52976cf4cfe4674f4b7b591cb` 的视频海报产品行为。两项均按 Toonflow 架构独立实现，huobao-drama 无许可证，未复制其源码。

- 分镜新增动作拍点 JSON 和人工确认状态，旧数据迁移后默认未确认；规范化会去空、去重并限制为最多 4 条、每条 160 字符。
- 生产 Agent 每个分镜写入单位生成 2–4 条有序动作/状态变化候选，不允许添加原分镜没有的人物、道具、台词或剧情，并且不能自行确认。
- 分镜卡显示拍点数量及待确认/已确认状态；编辑弹窗支持逐行修改和人工确认，没有拍点时不能确认。
- 单条与批量视频提示词只把已确认拍点编译为有序阶段约束；未确认、空值和旧分镜完全保持原 `videoDesc` 行为。四个运行时模型模板和后备提示词都覆盖此契约，XML 属性同时安全转义。
- 视频保存成功后在后台用本机 FFmpeg 生成 `.poster.jpg` 海报，0.5 秒失败回退 0 秒；同文件去重、最多两个并发、partial 原子落盘。历史列表发现缺失时按需补齐，失败不影响视频成功。
- 视频工作台优先显示海报图，缺失时保留浏览器首帧回退；同时统一兼容后端“生成成功”和前端“已完成”状态，避免重开后成功视频被显示为未生成。

验证：根 `yarn lint`、前端 Vite + `vue-tsc`、后端生产构建、知识地图/索引与 12 来源台账校验通过；动作拍点迁移在临时 SQLite 连续执行两次后字段存在且迁移记录保持 12 条；本地 1 秒视频成功提取 JPEG 海报。没有启动 Toonflow 服务，没有调用文本、图片、视频或 TTS 生成 API。

待真实运行验收：在实际项目中让生产 Agent 写入拍点，人工编辑确认后分别生成单条/批量提示词并检查模型是否按序消费；打开含大量历史视频的工作台观察首轮后台回填和列表流畅度；在未安装 FFmpeg 的机器上确认纯回退体验。以上都不阻塞静态合入。

## 下一批优先级

1. 从 12 个登记来源的新游标之后继续增量扫描；优先评估 capsule-cinema 的“代表镜头先审 + 配方版本 + 局部返工”，先明确资产引用、版本和许可清单。
2. 正式版本化 OpenAPI、独立 API Key 和 webhook 属于新的公网自动化安全边界，应单独设计权限、轮换、签名和重放保护后再实现。
3. wind-comic 视觉漂移阈值与节奏反馈、Remotion/HyperFrames、内置 FFmpeg 二进制仍等待数据契约或发行决策；真实付费试片、actual cost 和未公开取消 API 继续等待授权/契约。
