  ...
  const tables: TableSchema[] = [
    // 用户表
    {
      ...
      builder: (table) => {
        ...
        table.text("name");
        table.text("password");
        table.integer("must_change_password").notNullable().defaultTo(1);
        table.primary(["id"]);
        table.unique(["id"]);
      },
      initData: async (knex) => {
        await knex("o_user").insert([{ id: 1, name: "admin", password: await hashPassword("admin123"), must_change_password: 1 }]);
      },
    },
    //项目表
    {
    ...
    //分镜
    {
      ...
      builder: (table) => {
        ...
        table.text("actionBeats");
        table.integer("actionBeatsConfirmed").notNullable().defaultTo(0);
