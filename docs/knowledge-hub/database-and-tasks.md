...
用户可从左侧“任务中心 → 持久任务”查看这些字段的安全摘要，按项目、通道和状态筛选，并执行取消或重试。单条/批量视频、批量/单张资产图、批量分镜图和图片编辑画布生成使用该事实源；原 `o_tasks` 记录位于“历史任务”页签。单张资产图同步 `o_image`/`o_assets`，画布节点把 `generationTaskId` 保存进 `o_imageFlow.flowData`，用于重开后恢复轮询。

`script.assets.extract` 使用 `text` 通道和项目级资源占用，一次任务固定保存当前项目与去重后的剧本 ID。模型输出中的关联 ID 在事务写入前与本批 ID 取交集；越过同步文本调用边界后的未知错误进入 `manual_review`，不会自动重复调用模型。任务状态同步回 `o_script.extractState/errorReason`。
...
- `script_import_batches`：一次正式导入的版本头，保存项目、来源类型、外部项目 ID/版本、原始 JSON 规范化校验和、章节选择校验和、导入器版本、状态和报告。
- `script_import_items`：每个外部章节/分集的映射与快照，保存外部 ID/顺序、对应 `o_script`、内容校验和和本次动作（新增、更新、未变化）。

同一项目、来源、外部项目、来源校验和和选择校验和具有唯一约束。新来源版本按外部章节 ID 查找现有剧本，事务内只更新变化内容并追加项目事件；导入历史保留脚本快照，因此删除当前剧本后仍能解释过去导入记录，后续再导入会新建映射。
