...
当前真实边界：持久任务已接入单条和批量视频生成，普通火山引擎支持 job ID 续跑；任务中心可查看、筛选、取消、重新入队并处理 `manual_review`。图片、TTS、合成和 QA 尚未接入；其他 vendor 仍多把提交和轮询封装为一次调用，`manual_review` 是状态不明时防重复付费的安全停点。可维护前端源码已从 Toonflow-web `9c4cb0e` 纳入 `frontend/`，`data/web` 只能由构建同步。不要把计划中的逐句 TTS、自动合成、QA、费用账本 UI 描述成已完成。
