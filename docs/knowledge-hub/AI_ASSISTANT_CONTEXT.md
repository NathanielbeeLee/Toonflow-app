...
当前真实边界：持久任务已接入单条/批量视频、批量资产图、批量分镜图、逐句 TTS、本地 FFmpeg 成片和媒体 QA；普通火山引擎、OpenAI 标准接口、MiniMax 海螺和可灵视频支持远端任务续跑，任务中心可操作任务并配置限流、预算和价格规则。“配音与字幕”支持角色音色、台词、字幕、声音素材、规范化时间线、六轨高清/低清成片、QA 和成片审核。价格只能来自用户规则，usage ledger 是预估预留而非供应商实际账单；单张图片持久任务仍未完成。账号使用 scrypt，默认密码首次登录强制修改，服务默认只监听 127.0.0.1，诊断导出不含秘密或业务内容。`manual_review` 是状态不明时防重复付费的安全停点，本地 FFmpeg/QA 不使用该付费边界。`data/web` 只能由 `frontend/` 构建同步。
