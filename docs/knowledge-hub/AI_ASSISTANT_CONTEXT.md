...
当前真实边界：持久任务已接入单条/批量视频、批量资产图和批量分镜图；普通火山引擎、OpenAI 标准接口、MiniMax 海螺和可灵支持远端任务续跑，任务中心可操作任务并配置供应商/模型限流。单张图片编辑入口、TTS、合成和 QA 尚未接入；其余视频 vendor 仍多把提交和轮询封装为一次调用。`manual_review` 是状态不明时防重复付费的安全停点。CLIProxyAPI 当前只列出文本与 GPT Image 模型，未配置可用视频凭据，不能描述为已实测视频生成。`data/web` 只能由 `frontend/` 构建同步。
