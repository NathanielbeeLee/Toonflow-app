...
当前真实边界：持久任务已接入单条/批量视频、批量资产图、批量分镜图和逐句 TTS；普通火山引擎、OpenAI 标准接口、MiniMax 海螺和可灵视频支持远端任务续跑，任务中心可操作任务并配置供应商/模型限流。“配音与字幕”页面已支持角色音色、确定性台词解析、逐句入队、字幕 cue 与 SRT/VTT；OpenAI 标准供应商已支持 `/audio/speech` 真实 TTS，其他供应商仍多为空占位。单张图片编辑、规范化时间线、合成和 QA 尚未接入；其余视频 vendor 仍多把提交和轮询封装为一次调用。`manual_review` 是状态不明时防重复付费的安全停点。CLIProxyAPI 当前只读模型列表没有视频模型，不能描述为已实测视频生成；TTS 也取决于代理是否转发 `/audio/speech`。`data/web` 只能由 `frontend/` 构建同步。
