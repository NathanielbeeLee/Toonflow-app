...
分镜以 `<storyboardItem>` 标签传入，**每个 `<storyboardItem>` 代表一「组」**，包含视频描述、已确认动作拍点和总时长：
...
| `actionBeats` | 已经人工确认的有序动作/状态变化；空字符串表示未确认或没有拍点 | 用户确认后由系统填写 |
...
- `actionBeats` 非空时，它是对 `videoDesc` 的顺序约束：镜头动作必须依次消费全部拍点，不得跳过、交换或并行为同一瞬间。
- 每个拍点只描述一个主要动作或状态变化；可根据该组总时长合理分配节奏，但不把阶段标签原样写入最终提示词。
- 拍点不得扩写成 `videoDesc` 中不存在的新人物、新道具、新台词或新剧情。
- `actionBeats` 为空时，完全按原有 `videoDesc` 解析和生成路径执行。
