...
- 制作工作台在单条或批量生成视频前先运行镜头准备度检查，集中提示空提示词、缺失参考素材、不受支持的时长/分辨率、未启用供应商或已有运行任务。
- 同一套门禁也在服务端预算预留和供应商提交前执行，避免绕过页面后产生无效或错误配置的付费请求。
- 生产 Agent 可为分镜生成有序动作拍点候选；在分镜卡中编辑并人工确认后，视频提示词才会按顺序执行这些动作，未确认内容不影响原流程。
- 视频候选列表优先显示本地海报帧，历史视频会按需补齐；海报失败仍可播放原视频，重开工作台也能正确识别已成功视频。
