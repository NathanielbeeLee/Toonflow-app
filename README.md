...
- 单条和批量视频生成统一进入持久任务队列，页面请求结束后任务仍有明确状态，并限制同时生成数量，减少供应商拥堵。
- 重复点击同一镜头会复用正在执行的任务；应用异常退出后会按付费边界安全恢复，远端状态不明确时停止自动重提，避免重复扣费。
- 火山引擎视频任务会保存远端任务编号；应用重启后继续查询原任务，不会重新提交同一付费请求。
- 任务中心新增“持久任务”页签，可按项目、通道和状态筛选并自动刷新，查看远端任务编号、重试次数和错误说明；支持安全取消、重新入队及人工确认后重试，同时保留原历史任务页签。
- 批量资产图和分镜图也进入持久任务队列，应用重启后不会被误判为失败；任务中心可设置每个供应商/模型的并发、RPM 和请求冷却时间。
- 单张角色、场景、道具图片以及图片编辑画布中的工作流生成也进入持久队列；页面关闭后可在任务中心继续查看，重新打开已保存画布会恢复结果轮询。
- 剧本页可从 AI Novel Writing Assistant 的小说章节或成品短剧 JSON 导入：支持粘贴、文件和本机 API，先预览角色、章节及“新增/更新/无变化”动作，再选择分集正式导入；同一版本重复操作不会产生重复剧本。
- 剧本提取资产时，角色和道具名称仅多出空白或括号定位词会复用项目已有资产，减少“林小雨”与“林小雨（主角）”这类重复项；成片渲染前会一次列出本地文件已丢失的镜头，便于重新生成或选片。
- 剧本资产提取现在进入持久任务队列，严格限制在当前项目和所选剧本；页面关闭或应用重启后仍有可追踪状态，付费调用状态不明确时不会自动重复提交。
- OpenAI 标准供应商新增 GPT Image 与异步视频接口，兼容官方 Sora 流程及 CLIProxyAPI 视频路由；代理侧仍需具备对应视频凭据才能实际生成。
- MiniMax 海螺视频任务现在会保存远端任务编号；应用重启或轮询暂时失败后继续查询原任务，不会自动重复提交。
- 可灵文生、图生、多图和 Omni 视频任务也支持保存远端身份并在重启后续跑；恢复查询只允许预定义接口路径。
- 新增“配音与字幕”工作区，可从剧本导入逐句对白/旁白、分配角色音色、单句或批量进入持久配音队列，并编辑、锁定和导出 SRT/WebVTT 字幕。
- OpenAI 标准供应商已接通 GPT-4o mini TTS、TTS-1 和 TTS-1 HD，可从可用模型/音色下拉框配置；其他仍为空实现的供应商会明确失败，不会伪装成成功音频。
- 逐句音频生成完成后会用本地 ffprobe 读取真实时长并重建未锁定字幕；“规范化时间线”可保存可复现版本，分别展示视频、原生声音、对白、旁白、SFX、环境声、BGM 和字幕轨道。
- 规范化时间线可进入本地 FFmpeg 持久渲染队列，输出低清预览或高清成片；支持多镜头裁剪/统一画幅、视频原声、对白、旁白、音效、环境声、BGM、对白 ducking、两遍响度/true peak 标准化和安全区字幕烧录，页面可直接播放和下载。
- 完成的成片可运行本地持久媒体 QA，检查编码/尺寸/时长、黑帧、冻结帧、静音、响度、true peak、字幕阅读速度与安全区，并在页面给出问题级别和时间点。
- 任务中心可配置项目预算和供应商模型单价；已知价格在视频、图片和配音入队前预留并阻止超额，未知价格可选择禁止。成片可记录审核通过/退回及意见，审核绑定输出校验和。
- 高清成片通过媒体 QA 且审核批准后，可进入持久发布队列生成交付 ZIP；压缩包包含 MP4、SRT/WebVTT 字幕、时间线、QA 与审核凭证、清单和校验和，页面关闭后仍会继续打包并可回来下载。
- 发布包使用已审核时间线的字幕快照，并在打包前重新核对磁盘成片校验和；每次审核生成独立交付物，新增审核或退回修改会撤销旧 ZIP，避免过期版本继续下载。
- 本地账号密码使用 scrypt 加盐哈希，首次登录强制修改默认密码；服务默认只监听本机。任务中心可导出不包含密钥、项目名称、剧本或生成内容的脱敏诊断 JSON。
- 新增项目指南、故障排查和可持续更新的功能知识中心，便于快速了解操作流程与当前能力边界。
